...
import {
    Server, Download, CheckCircle, Menu, X, Home, FileText,
...
export function meta({}: Route.MetaArgs) {
    return [
        ...
        { name: "description", content: "Professional WHM API automation system for auto add package, create cPanel accounts, install LiteSpeed, SMTP integration." },
        { name: "keywords", content: "whm api, addpackage, addpkg, script addpkg, auto install litespeed, whm api create account, smtp whm api, whm api automation, whmcpanel" },
        { name: "author", content: "NdraDev" },
        { name: "robots", content: "index, follow" },
        { property: "og:type", content: "website" },
        { property: "og:title", content: "WHM API Automation V5" },
...
export default function HomePage({ loaderData }: Route.ComponentProps) {
    ...
    const fetchStats = async () => {
        ...
        try { const res = await fetch("/api/ip/list"); const data = await res.json(); if (data.success) { setServerCount(data.total || 0); setRegisteredIPs(data.data || []); } }
    ...
    const handleRegister = async () => {
        ...
        try {
            const check = await fetch(`/api/ip/check/${encodeURIComponent(ip.trim())}`); const checkData = await check.json();
    ...
    const faqs = [
        { q: "Apa itu WHM API?", a: "WHM API adalah interface untuk automasi manajemen server WHM/cPanel." },
        { q: "Cara menggunakan script addpkg?", a: "Script addpkg dapat dijalankan via API untuk membuat package baru." },
        { q: "Support LiteSpeed?", a: "Ya, support auto installation LiteSpeed Enterprise via API." },
        { q: "Ada SMTP integration?", a: "Ya, tersedia SMTP integration untuk email otomatis dari WHM." },
    ];

    const navItems = [{ icon: Home, label: "Home", href: "/" }, { icon: Server, label: "IP Server", href: "/ip-server-terdaftar" }, { icon: FileText, label: "API Docs", href: "/documentation" }, { icon: FileText, label: "Artikel", href: "/artikel" }];
        ...
        <div className="min-h-screen bg-background relative">
            ...
            <main className="container mx-auto px-4 py-10 relative z-10">
                <div className="max-w-6xl mx-auto space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><StatCard icon={Server} label="Servers Registered" value={serverCount} trend="+12%" delay={0.1} loading={statsLoading} /><StatCard icon={Users} label="Active Users" value="500+" trend="+25%" delay={0.2} loading={statsLoading} /><StatCard icon={BarChart3} label="API Calls/Day" value="10K+" trend="+18%" delay={0.3} loading={statsLoading} /></div>
